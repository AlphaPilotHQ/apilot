...
def test_core_import():
    """测试核心包的导入"""
    import apilot
    assert hasattr(apilot, "__version__")
    assert isinstance(apilot.__version__, str)
...
def test_spread_strategy_imports():
    ...
    assert SpreadStrategyTemplate
    assert SpreadBacktestingEngine


def test_portfolio_strategy_imports():
    """测试组合策略模块的导入"""
    ...
    assert PortfolioStrategyTemplate
    assert PortfolioBacktestingEngine
