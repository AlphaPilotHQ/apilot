...
from apilot.trader.constant import Direction, Exchange, Offset, OrderType
...
def main() -> None:
    ...
    try:
        ...
        for account in accounts:
            print(
                f"账户 {account.accountid}: 余额 = {account.balance}, 可用 = {account.available}"
            )
        ...
        # 下单
        print(
            f"下单: {req.symbol}, {'买入' if req.direction == Direction.LONG else '卖出'}, {req.type.value}, 数量 {req.volume}"
        )
