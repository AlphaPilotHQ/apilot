...
class CsvDatabase(BaseDatabase):
    ...
    def _get_file_path(self, symbol: str, exchange: Union[Exchange, str], interval: Interval) -> str:
        """
        根据交易对信息获取CSV文件路径
        """
        # 如果直接设置了CSV文件路径，则直接返回
        if self.csv_path:
            # 如果设置了直接文件路径，则直接返回
        ...
        # 否则尝试从数据目录查找文件
        file_pattern = f"{symbol}_{exchange_value}_{interval.value}*.csv"
    ...
    def load_bar_data(
        ...
        # 获取交易所值，用于日志显示
        exchange_value = exchange.value if hasattr(exchange, 'value') else exchange
        logger.info(f"CSV数据库: 请求加载 {symbol}.{exchange_value} 数据，区间 {interval}，时间 {start} 至 {end}")
