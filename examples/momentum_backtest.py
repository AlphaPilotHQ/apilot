...
def run_backtesting(
    ...
    logger.info(
        f"步骤3: 策略添加完成 - {strategy_class.__name__} 参数: std_period={std_period}, "
    ...
    # 加载回测数据
    engine.load_data()
    logger.info("步骤5: 数据加载完成")

    # 运行回测
    engine.run_backtesting()
    logger.info("步骤6: 回测完毕")
    ...
    if statistics:
        ...
        max_drawdown = statistics.get('max_ddpercent', 0)  # 使用正确的键名max_ddpercent
        total_trade_count = statistics.get('total_trade_count', 0)
        
        logger.info(
            f"回测结果 - 收益率: {total_return:.2f}%, "
            f"夏普比率: {sharpe_ratio:.2f}, "
            f"最大回撤: {max_drawdown:.2f}%, "
            f"交易次数: {total_trade_count}"
        )
    else:
        logger.warning("未能计算回测统计结果")
