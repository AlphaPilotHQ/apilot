...
class MongoDBDatabase(BaseDatabase):
    ...
    def load_bar_data(
        ...
        **kwargs,
        ...
        logger.info("Using hardcoded query approach from test script")

        # 1. Clear and reset filter_dict
        filter_dict = {"symbol": symbol_uppercase}
        ...
        filter_dict["kline_st"] = {"$gte": start_timestamp, "$lte": end_timestamp}
        ...
        if self.is_timestamp_ms or datetime_field == "kline_st":
            ...
            logger.debug(
                f"Timestamp query range (ms): {start_timestamp} to {end_timestamp}"
            )
        else:
            filter_dict[datetime_field] = {
                "$gte": start,
        ...
        try:
            ...
            if test_doc:
                ...
                if symbol_field in test_doc:
                    logger.debug(
                        f"Sample symbol in collection: {test_doc[symbol_field]}"
                    )
                if datetime_field in test_doc:
                    logger.debug(
                        f"Sample timestamp in collection: {test_doc[datetime_field]}"
                    )
        ...
        try:
            cursor = (
                self.collection.find(filter_dict, projection)
                .sort(datetime_field, ASCENDING)
                .limit(limit_count)
            )
            ...
            if count == 0:
                logger.warning("No matching records found, trying modified query")

                datetime_filter = {datetime_field: filter_dict[datetime_field]}
                symbols_count = self.collection.count_documents(
                    datetime_filter, limit=10
                )
                if symbols_count > 0:
                    sample_symbols = list(
                        self.collection.distinct(
                            self.field_map["symbol"], datetime_filter
                        )
                    )[:5]
                    logger.info(
                        f"Time-only query found {symbols_count} records, sample symbols: {sample_symbols}"
                    )
        ...
        try:
            ...
            if test_docs:
                logger.info(
                    f"Test query successful: got documents, sample: {test_docs[0]}"
                )
            ...
            try:
                ...
                def safe_float(val, default=0.0):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Cannot convert to float: {val}, using default: {default}"
                        )
                        return default
                ...
                bar = BarData(
                    ...
                    open_interest=safe_float(
                        doc.get(self.field_map["open_interest"], 0)
                    ),
        ...
        logger.info(
            f"Loaded {len(bars)} records from MongoDB for {symbol} in {elapsed:.2f} seconds"
        )
        return bars
